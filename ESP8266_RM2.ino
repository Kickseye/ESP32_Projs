...
const int switchPinRoom1 = D1;  // Room 1 Occupancy
const int switchPinRoom2 = D5;  // Room 2 Occupancy
const int switchPinFan = D6;    // Fan switch
...
void loop() {
    ...
    if (fanState != lastFanState && millis() - lastDebounceTimeFan > debounceDelay) {
        ...
        webSocket.sendTXT(fanState == LOW ? "FAN_ON_2" : "FAN_OFF_2");
    }
    ...
    if (lightState != lastLightState && millis() - lastDebounceTimeLight > debounceDelay) {
        ...
        webSocket.sendTXT(lightState == LOW ? "LIGHT_ON_2" : "LIGHT_OFF_2");
    }
